...
/* ─── Médiane (nécessite un tri interne) ───────────────────────── */ /* Fonction de comparaison pour qsort */ static int compare_doubles(const void *a, const void *b) {     double da = *(const double *)a;     double db = *(const double *)b;     return (da > db) - (da < db); }
...
double produit_scalaire(double *v1, double *v2, int n) {     
    double resultat = 0.0;     for (int i = 0; i < n; i++) {         
        resultat += v1[i] * v2[i];     
    }     
    return resultat;
}
